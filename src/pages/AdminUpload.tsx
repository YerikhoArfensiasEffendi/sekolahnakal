...
export default function AdminUpload() {
  ...
  // ================= 2.1 DISCORD TRANSACTIONS STATE =================
  ...
  return (
    <div className="min-h-screen bg-[#0c0c0c] text-white pt-16 flex">
          ...
          <nav className="space-y-1">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span>Transaksi & Log Discord</span>
              </div>
      ...
      <main className="flex-1 min-w-0 p-4 sm:p-6 lg:p-8 space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">
                ...
                {activeTab === 'discord_realtime' && '🧾 Riwayat Transaksi & Log Discord'}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="md:hidden">
              ...
              >
                ...
                <option value="discord_realtime">Transaksi & Log Discord</option>
        ...
        {/* ========================================================================= */}
        {/* ========================================================================= */}
        {/* TAB: RIWAYAT TRANSAKSI & LOG UPLOAD DISCORD (COMPACT & CLEAN) */}
        {/* ========================================================================= */}
        {activeTab === 'discord_realtime' && (
          <div className="space-y-6">
            {/* 1. Riwayat Transaksi Pembelian Discord (Ultra Compact & Minimal) */}
            <div className="rounded-xl border border-zinc-800 bg-[#0e0e11] overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-zinc-800/80 bg-zinc-900/30">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-zinc-400 font-mono ml-2">
                    ({discordPayments.length} Transaksi)
                  </span>
                </div>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold transition-all flex items-center gap-1.5 disabled:opacity-50 cursor-pointer shadow-sm"
                >
                  <span className={isLoadingPayments ? 'animate-spin' : ''}>🔄</span>
                  <span>{isLoadingPayments ? 'Menyinkronkan...' : 'Sinkronkan'}</span>
                </button>
              </div>

              {discordPayments.length === 0 ? (
                <div className="p-6 text-center text-xs text-zinc-500">
                  Belum ada riwayat transaksi tersinkron. Klik tombol "Sinkronkan" di atas.
                </div>
              ) : (
                <div className="overflow-x-auto max-h-[360px] overflow-y-auto">
                  <table className="w-full text-left text-xs">
                    <thead className="sticky top-0 bg-[#121215] border-b border-zinc-800 text-[10px] font-mono uppercase tracking-wider text-zinc-400 z-10">
                      <tr>
                        ...
                        return (
                          <tr key={p.id || p.orderId} className="hover:bg-white/[0.02] transition-colors">
                            <td className="py-2 px-3">
                              <div className="flex items-center gap-2">
                                <div className="h-6 w-6 rounded-full overflow-hidden bg-zinc-800 shrink-0 flex items-center justify-center">
                                  {p.avatarUrl ? (
                                ...
                                <span className="font-sans font-bold text-white text-xs">@{p.username}</span>
                              </div>
                            </td>
                            <td className="py-2 px-3">
                              <span
                            ...
                            <td className="py-2 px-3 text-[11px] text-zinc-400 select-all">
                              {p.orderId}
                            </td>
                            ...
                            <td className="py-2 px-3 text-right text-[11px] text-zinc-400 font-sans">
                              {p.timestamp ? new Date(p.timestamp * 1000).toLocaleString('id-ID', { dateStyle: 'short', timeStyle: 'short' }) : p.createdAt}
                            </td>
                          </tr>
                        );
                      })}
            ...
            {/* 2. Log Upload Video & Ingestion (Clean Normal Log View) */}
            <div className="rounded-xl border border-zinc-800 bg-[#09090b] overflow-hidden shadow-lg">
              <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-2.5 bg-[#121214] border-b border-zinc-800">
                <div className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block animate-pulse" />
                  <h3 className="text-xs font-mono font-bold text-zinc-200">
                    LOG AKTIVITAS UPLOAD & ARSIP VIDEO (LIVE FEED)
                  </h3>
                  <span className="text-[10px] font-mono text-zinc-500">
                    ({movies.length} Total Arsip)
                  </span>
                </div>

                <div className="flex items-center gap-2 text-xs font-mono">
                  <span className="text-[10px] text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded">
                    Cloud Scraper: Running
                  </span>
                </div>
              </div>

              {/* Monospace Normal Log Stream */}
              <div
                className="p-3 font-mono text-xs max-h-[320px] overflow-y-auto space-y-1 bg-[#09090b] text-zinc-300"
                style={{ scrollbarWidth: 'thin' }}
              >
                {movies.slice(0, 40).map((m, idx) => (
                  <div
                    key={m.id || idx}
                    className="flex flex-wrap items-baseline gap-2 py-1 px-2 rounded hover:bg-white/[0.03] transition-colors border-b border-white/[0.02]"
                  >
                    <span className="text-[11px] text-zinc-500 select-none">
                      [{(m as any).createdAt ? new Date((m as any).createdAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }) : 'LIVE'}]
                    </span>
                    <span className="px-1.5 py-0.2 rounded text-[10px] font-bold bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">
                      SUCCESS
                    </span>
                    <span className="px-1.5 py-0.2 rounded text-[10px] font-bold bg-cyan-500/15 text-cyan-300 border border-cyan-500/30">
                      [{m.genres?.[0] || 'Media'}]
                    </span>
                    ...
                    <span className="text-[11px] text-zinc-500 ml-auto hidden sm:inline truncate max-w-[200px]">
                      ZeroStorage CDN
                    </span>
                  </div>
                ))}
